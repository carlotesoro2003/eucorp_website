<script lang="ts">
	...
	/** Fetch risk monitoring data based on the user's department_id */
const fetchRiskMonitoring = async () => {
	try {
		...
		// Fetch the user's department_id from the profiles table
		...
		if (profileError || !userProfile?.department_id) {
			throw new Error("Unable to fetch user profile or department_id");
		}

		const departmentId = userProfile.department_id;

		// Fetch risk monitoring data filtered by department_id
		const { data, error } = await supabase
				...
				.select(`
					...
					is_achieved
				`)
				.eq("risks.department_id", departmentId);

			if (error) throw error;

			// Ensure risks field is not null and map data
			risksMonitoring = data
				.filter((item: any) => item.risks !== null) // Exclude null risks
				.map((item: any) => ({
					id: item.id,
