...
<div class="w-full rounded-lg border bg-card border-border p-4 hover:shadow-lg transition-all duration-300">
	<h2 class="mb-6 text-xl font-semibold">Risk Analysis</h2>
	...
	{:else}
		...
		<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
			<p class="font-medium text-sm">Classification:</p>
			<div class="relative w-full sm:w-1/3 lg:w-1/4">
				<select bind:value={selectedCategory} onclick={createChart} class="appearance-none bg-secondary border-secondary rounded-lg px-4 py-2 pr-10 text-sm font-medium focus:outline-none transition-colors focus:ring-2 focus:ring-ring w-full md:w-[150px]">
